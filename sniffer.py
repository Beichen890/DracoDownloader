...
_QUOTED_URL_RE = re.compile(
    r'''["']((?:https?:)?//[^\s"'<>\)]+\.(?:m3u8|mp4|webm|mkv|avi|mov|flv|mp3|aac|flac|ogg|wav|zip|rar|7z|tar|gz|pdf|torrent)(?:\?[^\s"'<>\)]*)?)["']''',
    re.IGNORECASE
)

# 匹配 source 标签 src（HTMLParser 已处理，此为补充，捕获 HTMLParser 可能漏掉的）
# 匹配 data-src / data-url 等 data 属性中的媒体 URL
_DATA_ATTR_RE = re.compile(
    r'''data-(?:src|url|video|audio|source)=["']((?:https?:)?//[^\s"'<>\)]+)["']''',
    re.IGNORECASE
)
    ...
    def extract(self, url: str, text: str) -> List[SniffedResource]:
        ...
        for m in _DATA_ATTR_RE.finditer(text):
            ...
            if u not in seen:
                ...
                results.append(SniffedResource(
                    url=urljoin(url, u),
                    type=classify_url(u) or ResourceType.UNKNOWN,
...
class ResourceSniffer:
    ...
    async def sniff(self, url: str,
        ...
        result.original_url = final_url  # 跟随重定向后的最终 URL

...
ANTI_BOT_STATUS_CODES = {401, 403, 429}
# 403/429 可重试（带退避），401 通常需登录态不自动重试
...
class HttpClient:
    ...
    async def fetch_text(self, url: str,
                         headers: Optional[Dict[str, str]] = None,
                         timeout: Optional[float] = None,
                         max_size: int = 5 * 1024 * 1024) -> Tuple[str, str, str]:
        """拉取文本内容
    ...
    async def fetch_bytes(self, url: str,
                          headers: Optional[Dict[str, str]] = None,
                          timeout: Optional[float] = None) -> Tuple[bytes, str, str]:
        """拉取二进制内容
